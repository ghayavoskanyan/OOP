...
enum class StatementType { ExprStatement, IfNode, WhileNode, ForNode, BlockNode, SeqNode, PrintNode, DeclNode };
...
class DeclNode : public StatementNode {
    ...
    SymbolTable& symbolTable;
public:
    DeclNode(const std::string& name, std::unique_ptr<ASTNode> init, SymbolTable& sym);
    int compile(std::vector<Instruction>& prog) const override;
