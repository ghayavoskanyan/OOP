...
StmtInterpreter::StmtInterpreter(SymbolTable& globals) : globals_(globals) {}
...
int32_t StmtInterpreter::evalExpr(const ASTNode* node, std::unordered_map<std::string, int32_t>* locals) {
    ...
    switch (node->type) {
        ...
        case NodeType::AssignmentNode: {
            ...
            setVar(globals_, locals, a->getVarName(), v);
            return v;
        }
        case NodeType::CallNode: {
            ...
            int32_t ret = 0;
            bool returned = false, brk = false;
            execStmt(fn->getBody(), &frame, ret, returned, brk);
...
void StmtInterpreter::execStmt(const StatementNode* stmt, std::unordered_map<std::string, int32_t>* locals,
                                 int32_t& returnValue, bool& returned, bool& breakFlag) {
    if (!stmt || returned || breakFlag) return;
    ...
    switch (stmt->type) {
        ...
        case StatementType::DeclNode: {
            ...
            setVar(globals_, locals, d->getVarName(), v);
            return;
        }
        case StatementType::ReturnStmt: {
        ...
        case StatementType::IfNode: {
            ...
            if (c) execStmt(i->getThen(), locals, returnValue, returned, breakFlag);
            else if (i->getElse()) execStmt(i->getElse(), locals, returnValue, returned, breakFlag);
            return;
        }
        case StatementType::WhileNode: {
            auto* w = static_cast<const WhileNode*>(stmt);
            while (!returned && !breakFlag) {
                if (!evalExpr(w->getCondition(), locals)) break;
                execStmt(w->getBody(), locals, returnValue, returned, breakFlag);
            }
            return;
        }
        case StatementType::ForNode: {
            ...
            while (!returned && !breakFlag) {
                ...
                execStmt(f->getBody(), locals, returnValue, returned, breakFlag);
                if (returned || breakFlag) break;
                if (f->getUpdate()) evalExpr(f->getUpdate(), locals);
            }
            return;
        }
        case StatementType::BlockNode: {
            for (const auto& s : static_cast<const BlockNode*>(stmt)->getStatements()) {
                execStmt(s.get(), locals, returnValue, returned, breakFlag);
                if (returned || breakFlag) return;
            }
            return;
        }
        case StatementType::SeqNode: {
            for (const auto& s : static_cast<const SeqNode*>(stmt)->getStatements()) {
                execStmt(s.get(), locals, returnValue, returned, breakFlag);
                if (returned || breakFlag) return;
            }
            return;
        ...
        case StatementType::SwitchStmt: {
            ...
            for (const auto& arm : sw->getArms()) {
                ...
                for (const auto& st : arm.statements) {
                    execStmt(st.get(), locals, returnValue, returned, breakFlag);
                    if (breakFlag) {
                        breakFlag = false;
                        return;
                    }
...
int32_t StmtInterpreter::run(const StatementNode* root) {
    ...
    auto runSeq = [&](const SeqNode* seq) {
        for (const auto& s : seq->getStatements()) {
            if (s->type == StatementType::FunctionDef) continue;
            execStmt(s.get(), nullptr, ret, returned, brk);
            if (returned || brk) break;
        }
    };

    if (root->type == StatementType::SeqNode)
        runSeq(static_cast<const SeqNode*>(root));
    else
        execStmt(root, nullptr, ret, returned, brk);
    ...
    if (it != functions_.end()) {
        std::unordered_map<std::string, int32_t> frame;
        execStmt(it->second->getBody(), &frame, ret, returned, brk);
    }
    return ret;
}
